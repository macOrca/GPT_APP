...
def response_chatbot(problem_description, sample_code, input_example, output_example, code, execution_result, feedback, user_profile):
    ...
    内容を更新しました。上記の内容を考慮し、ユーザの質問に答えてください。ユーザは高校生を想定し、高校生が理解できるように簡潔で専門用語の少ない内容で回答してください。また回答する際に、最低限の文章量で直接正解を示さず、ユーザが正しい回答にたどり着くためのヒントを示してください。
    """
...
def analyze_submission(problem_description, sample_code, input_example, output_example, code, execution_result, user_profile):
    ...
    上記の内容に基づいて、以下の指示に従ってください。高校生にもわかるように、専門用語を使わず、平易で要点を絞った表現にしてください。まず、問題の理解、アルゴリズム設計、コード実装、実行結果の4点について、ユーザの理解状態を推測してください。次に、ユーザの誤った理解の内容を示し、原因を推測してください。加えて、解答について、「正解」、「不正解」のどちらかのみを回答してください。最後に、提出コードの良い点と誤った点を150字を超えないようにフィードバックしてください。回答にあたっては学習者の理解状態を考慮し、良い点は褒め、誤った点については誤りを含む場所を示し、直接の正解を示さず、ユーザが正解にたどり着くためのヒントのみを示してください。
...
def analyze_user_feedback(feedback, user_profile):
    ...
    専門用語を使わず、平易で要点を絞った表現にしてください。このフィードバックと前回の学習者モデルの内容を統合し、抜けがないように注意して、ユーザが理解できている内容、理解に誤りがある内容を明確に箇条書きで提示してください。
    """
    response = client.beta.chat.completions.parse(
        model=gpt_model,
        messages=[
